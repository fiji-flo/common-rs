...
/// a log watcher that can make assertions about the tracing logs that occurred
    ...
    E: 'static,
{
    ...
    ///
    /// ```
    /// # use crate::utils::{LogWatcher, MozLogMessage};
    /// # use std::sync::{Arc, Mutex};
    /// # use serde_json::json;
    /// # let mut log_watcher = LogWatcher::with_events(vec![
    /// #     MozLogMessage {
    /// #         fields: maplit::hashmap!{
    /// #             "message".to_string() => json!("request success")
    /// #         },
    /// #         ..Default::default()
    /// #     }
    /// # ]);
    /// #
    /// assert!(log_watcher.has(|msg|
    ///     msg.fields.get("message").map(|v| v.as_str()) == Some(Some("request success"))
    /// ));
    /// ```
    ...
    /// objects into event instances that are placed in `self.events`.
    fn convert_events(&mut self) {
        ...
        while let Some(idx) = log_text.find('\n') {
            ...
            // Now `message_json` contains the first line of logs, and `log_text` contains the rest.
